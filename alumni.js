...
router.get("/",(rep,res)=>{
  ...
  conn.query('SELECT * FROM  alumni', function (error, results, fields) {
    // Handle errors
...
// router.get("/", (rep, res) => {
//   conn.query(
//     `SELECT b.id, b.name, b.description, b.pages, b.cover, b.aqris, b.tijaabi, b.price, b.catagory, b.status as status_id, s.status as status, c.cat_name as catagory_name  FROM alumni b
...
router.get("/:id",(rep,res)=>{
  ...
  conn.query('SELECT * FROM  alumni WHERE id = ?', [rep.params.id], function (error, results, fields) {
    // Handle errors
    if (error) throw error;
...
router.get("/login/:aluID", (req, res) => {
  conn.query(
    'SELECT * FROM alumni  WHERE univ_id = ?',
    [req.params.aluID],
    function (error, results, fields) {
      // Handle errors
    if (error) throw error;
...
router.post("/", (req, res) => {
  const emp = req.body;
  conn.query("INSERT INTO alumni SET ?", emp, function (error, results, fields) {
    // Handle errors
    if (error) {
