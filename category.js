...
// router.get("/", (rep, res) => {
//   conn.query(
//     `SELECT b.id, b.name, b.description, b.pages, b.cover, b.aqris, b.tijaabi, b.price, b.catagory, b.status as status_id, s.status as status, c.cat_name as catagory_name  FROM category b
...
router.get("/:id",(rep,res)=>{
  ...
  conn.query('SELECT * FROM  category WHERE id = ?', [rep.params.id], function (error, results, fields) {
    // Handle errors
    if (error) throw error;
...
router.post("/", (req, res) => {
  const emp = req.body;
  conn.query("INSERT INTO category SET ?", emp, function (error, results, fields) {
    // Handle errors
    if (error) {
