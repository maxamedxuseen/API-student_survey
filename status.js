...
router.get("/",(rep,res)=>{
  ...
  conn.query('SELECT * FROM status', function (error, results, fields) {
    // Handle errors
...
// router.get("/", (rep, res) => {
//   conn.query(
//     `SELECT ch.id, ch.chepter_name, ch.book, ch.maqal, b.name FROM status ch
...
//   conn.query(
//     `SELECT ch.id, ch.chepter_name, ch.book, ch.maqal, b.name,b.cover FROM status ch
//     INNER JOIN books b on ch.book = b.id
//     WHERE b.id = ?`,
//     [rep.params.id],
//     function (error, results, fields) {
